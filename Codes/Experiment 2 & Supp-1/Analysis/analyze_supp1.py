...
def format_method_name(name):
    """
    Formats method names like 'ULM_M1p2' to 'M=1.2'.
    """
    if name.startswith('ULM_M'):
        val = name[5:].replace('p', '.')
        return f"M={val}"
    return name
...
def save_combined_plot(intensity_res, clarity_res, rt_matrix, output_path):
    fig, axes = plt.subplots(1, 3, figsize=(20, 6), sharey=False, gridspec_kw={'width_ratios': [1, 1, 1.3]})
    title_y = 1.02
...
def main():
    ...
    data_dir = os.path.join(base_dir, "Data", "Supplementary_Experiment_1")
    output_dir = os.path.join(base_dir, "Analysis", "Supplementary_Experiment_1")
    ...
    save_combined_plot(intensity_for_plot, clarity_for_plot, rt_matrix, os.path.join(output_dir, "Supplementary_Experiment_1_Combined"))
    ...
    overall_payload = {
        ...
        'output_files': {
            'combined_plot_pdf': os.path.join(output_dir, "Supplementary_Experiment_1_Combined.pdf"),
            'combined_plot_svg': os.path.join(output_dir, "Supplementary_Experiment_1_Combined.svg"),
            'combined_plot_png': os.path.join(output_dir, "Supplementary_Experiment_1_Combined.png"),
            ...
            'overall_detail_json': os.path.join(output_dir, "Supplementary_Experiment_1_overall_detailed_results.json")
        }
    }
    save_structured_results(output_dir, "Supplementary_Experiment_1_overall_detailed_results.json", overall_payload)

    # Simplified summary for paper
    simplified_summary = {}

    def extract_block_summary(block_res):
        if block_res is None or block_res[3] is None:
            return None
        payload = block_res[3]
        return {
            ...
            'pairwise_p_values': [
                {
                    'pair': f"{item['method_1']} vs {item['method_2']}",
                    'p_value': item['p_value'],
                    'significance': item['significance']
                }
                for item in payload.get('pairwise_wald', [])
            ]
        }

    simplified_summary['intensity'] = extract_block_summary(res_intensity)
    simplified_summary['clarity'] = extract_block_summary(res_clarity)
    
    # Add reaction time summary
    if rt_matrix_details and 'pair_means' in rt_matrix_details:
         simplified_summary['reaction_time'] = {
             'pair_means': [
                 {
                     'pair': f"{item['stimulus_1']} vs {item['stimulus_2']}",
                     'zlogrt': item['mean_zlogrt']
                 }
                 for item in rt_matrix_details['pair_means']
             ]
         }

    save_structured_results(output_dir, "Supplementary_Experiment_1_summary.json", simplified_summary)
