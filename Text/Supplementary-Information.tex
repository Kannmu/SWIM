...
The model targets the Pacinian corpuscle (PC) pathway, which mediates detection thresholds for vibrotactile stimuli above \qty{100}{\hertz}~\citep{johnsonRolesFunctionsCutaneous2001, handlerMechanosensoryNeuronsTouch2021}. The \qty{200}{\hertz} carrier envelope aligns precisely with peak PC sensitivity. While ultrastructural studies implicate Lamellar Schwann Cells (LSCs) in tuning PC dynamic sensitivity~\citep{ziolkowskiTouchSensationRequires2025}, explicitly modeling LSC-neurite biophysics introduces unconstrained parameters. We instead abstracted the inner core complex into an effective spatiotemporal integration kernel (Eq. 46) to represent the end-organ's summation of coherent mechanical events.
